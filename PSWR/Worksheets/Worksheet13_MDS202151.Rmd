...
#The test statistic is t= 137.97 and the degrees of freedom is 11 
...
#Since the p-value<0.05 , we reject the null hypothesis in this case.
...
qqline(x1,col='red')
qqnorm(x2,xlab='t-ditribution with df=25')
qqline(x2,col='red')

#We can see both the normal qq plots are almost the same, but near the
#end both start deviating from the qqline.
```
...
#start to overlap with the dashed line representing normal distribution.
...
#sample mean. The claim made by Doddapple can not be believed.
